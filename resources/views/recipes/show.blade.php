    ...
    <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
      <dl class="sm:divide-y sm:divide-gray-200">
        ...
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
            <div class="border border-gray-200 rounded-md pl-3 pr-4 py-3 flex recipes-center justify-between text-sm">
            <div class="w-0 flex-1 flex recipes-center">
            <img src="{{$recipe->item->icon}}" class="flex-shrink-0 h-5 w-5" aria-hidden="true" />
            <span class="ml-2 flex-1 w-0 truncate">  {{$recipe->item->name}} </span>
            </div>
            <div class="ml-4 flex-shrink-0">
            <a href="/items/{{$recipe->item->id}}" class="font-medium text-indigo-600 hover:text-indigo-500"> Open </a>
            </div>
          </div>
          </dd>
      </div>
                ...
                @foreach ($recipe->ingredients as $ingredient)
                    <li class="pl-3 pr-4 py-3 flex recipes-center justify-between text-sm">
                        ...
                        <!-- Heroicon name: solid/paper-clip -->
                          <img src="{{$ingredient->icon}}" class="flex-shrink-0 h-5 w-5" aria-hidden="true" />
                          <span class="ml-2 flex-1 w-0 truncate">  {{$ingredient->name}} - {{$ingredient->pivot->count}} </span>
                        </div>
                        <div class="ml-4 flex-shrink-0">
                        <a href="/items/{{$ingredient->id}}" class="font-medium text-indigo-600 hover:text-indigo-500"> Open </a>
                        </div>
                    </li>
